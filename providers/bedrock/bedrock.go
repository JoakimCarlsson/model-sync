...
// Fetch retrieves the price list.
func (p *Provider) Fetch(ctx context.Context) ([]catalog.Document, error) {
	if body, ok := p.readCache(); ok {
		return []catalog.Document{{URL: PriceListURL, Body: body}}, nil
	}
	...
	if err != nil {
		return nil, fmt.Errorf("fetch %s: %w", PriceListURL, err)
	}
	defer func() { _ = resp.Body.Close() }()
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("fetch %s: %s", PriceListURL, resp.Status)
	}
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("read %s: %w", PriceListURL, err)
	}
	p.writeCache(body)
	return []catalog.Document{{URL: PriceListURL, Body: body}}, nil
}

// Parse decodes the price list.
